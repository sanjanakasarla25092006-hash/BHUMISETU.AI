...
const regex = /function headerHtml\(activeTop\)\{[\s\S]*?<\/header>\n\s*`;\n\}/;
...
const newHeader = `function headerHtml(activeTop){
  ...
  <header style="width:100%; position:sticky; top:0; z-index:50; box-shadow:0 4px 20px rgba(0,0,0,0.15);">
    ...
    <div style="background: linear-gradient(135deg, #0a1128, #1b2a41); color: #fff; padding: 24px 0; border-bottom: 2px solid var(--saffron-500);">
      <div class="container" style="display: flex; justify-content: space-between; align-items: center; position: relative; min-height: 85px;">
        ...
        <div style="flex: 1; display: flex; justify-content: flex-end; align-items: center; gap: 16px;">
          ...
          <button data-action="lang" style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: #fff; padding: 6px 12px; border-radius: 4px; font-size: 13px; font-weight: 600; cursor: pointer;">A/\u0905 Language</button>
        </div>
...
if (regex.test(code)) {
    ...
    console.log("Could not find headerHtml using regex.");
}
