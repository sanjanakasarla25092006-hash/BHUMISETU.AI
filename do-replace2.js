...
function pageSignin(){
  ...
  setTimeout(() => renderDynamicLoginFields(document.querySelector('input[name="loginRole"]:checked').value), 50);
...
const regex2 = /function promptSmsKey\(\)\s*\{[\s\S]*?\}\s*function pageSignin/;

if (regex2.test(code)) {
    code = code.replace(regex2, dynamicLogin + "\nfunction pageSignin_dummy");
    code = code.replace(/\nfunction pageSignin_dummy\(\)\s*\{[\s\S]*?\${footerHtml\(\)}`;\s*\}/, "");
