...
if (startIndex !== -1 && endIndex !== -1) {
  const finalCode = code.substring(0, startIndex) + newHeader + code.substring(endIndex + endStr.length);
  fs.writeFileSync('index-4.html', finalCode, 'utf8');
  console.log("Successfully replaced headerHtml!");
} else {
  console.log("Failed to find headerHtml with endStr: " + endStr);
  console.log("Trying alternative...");
  const endStr2 = "</header>`;\r\n}";
  const endIndex2 = code.indexOf(endStr2, startIndex);
  if (startIndex !== -1 && endIndex2 !== -1) {
    const finalCode2 = code.substring(0, startIndex) + newHeader + code.substring(endIndex2 + endStr2.length);
    fs.writeFileSync('index-4.html', finalCode2, 'utf8');
    console.log("Successfully replaced headerHtml using CRLF!");
  } else {
